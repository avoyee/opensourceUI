...
#if USE_SWAP_BUFF
    char idx = 0;
    unsigned long long pix0;
    unsigned long long pix1;
    RECT updateRect = {0};
    RECT updateRect1 = {0};
    pix0 = (this->hidden->realfb_info->phy_addr);
    pix1 = (this->hidden->realfb_info->phy_addr) +
           (this->hidden->realfb_info->pitch * this->hidden->realfb_info->height);
#endif
    //framesize = this->hidden->realfb_info->pitch * this->hidden->realfb_info->height;
    ...
    for(;;) {
        ...
        if(_shadowfbheader->dirty || _shadowfbheader->palette_changed) {
            ...
            if(real_device) {
                ...
                if(idx == 0) {
                    this->hidden->realfb_info->phy_addr = pix1;
                    CopyRect(&updateRect, &_shadowfbheader->dirty_rect);
                    GetBoundRect(&_shadowfbheader->dirty_rect, &_shadowfbheader->dirty_rect, &updateRect1);
                } else {
                    this->hidden->realfb_info->phy_addr = pix0;
                    CopyRect(&updateRect1, &_shadowfbheader->dirty_rect);
                    GetBoundRect(&_shadowfbheader->dirty_rect, &_shadowfbheader->dirty_rect, &updateRect);
                }
            ...
            if(idx == 0) {
                if(real_device->UpdateFbOffsety) {
                    real_device->UpdateFbOffsety(real_device, this->hidden->realfb_info->height);
                    idx = 1;
                }
            } else {
                if(real_device->UpdateFbOffsety) {
                    real_device->UpdateFbOffsety(real_device, 0);
                    idx = 0;
                }
            }
