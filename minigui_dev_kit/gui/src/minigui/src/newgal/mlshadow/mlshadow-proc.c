...
static int MLSHADOW_VideoInit (_THIS, GAL_PixelFormat *vformat)
{
    ...
    if (mgIsServer && (this->hidden->_real_device == NULL)) {
        ...
        this->hidden->_real_def_bgcolor = PIXEL_red;
        if(this->hidden->_real_def_bgcolor > 0xffffff ||
                this->hidden->_real_def_bgcolor < 0)
        {
            this->hidden->_real_def_bgcolor = 0;
        }
		printf("%s %d %x %x %x %x\n",__FUNCTION__,__LINE__,real_vformat.BitsPerPixel, real_vformat.Rmask,
                real_vformat.Gmask, real_vformat.Bmask);
        ...
        if(!_mmaped_flag){
            ...
            if(!reply.ret_value) {
                ...
                if(video_mem_info.type == MLS_FB_TYPE_VIDEOMEM){
                    ...
                    _mmaped_mem = mmap(NULL, video_mem_info.video_mem_len,
                            PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
                    fprintf(stderr, "_mmaped_mem=%0x, video_mem_info = %d\n", (unsigned int)_mmaped_mem, video_mem_info.video_mem_len);
                }
            }
        }
    }
                ...
                int width, int height, int bpp, Uint32 flags)
{
    ...
    if (!GAL_ReallocFormat (current, bpp, 0xff, 0xff00, 0xff0000, 0xff000000)) {
        return (NULL);
    }
    ...
    if (mgIsServer && list_empty (&mlshadow_list)) {
        ...
        {
            fprintf (stderr, "NEWGAL>MLShadow: real_engine %s.\n",real_engine);
        }
        ...
        //    return (NULL);
        //}
        printf("%s %d %d %d %d \n",__FUNCTION__,__LINE__,w,h,depth);
    ...
    if (mgIsServer) {
        ...
        if(!reply.ret_value) {
            ...
            switch(header->type) {
                case MLS_FB_TYPE_VIDEOMEM :
                    if(video_mem_info.type == MLS_FB_TYPE_VIDEOMEM) {
						...
						printf("%s %d %d \n",__FUNCTION__,__LINE__,video_mem_info.video_pitch);
                    }
                    break;
			...
			printf("%s %d %d \n",__FUNCTION__,__LINE__,current->pitch);
        ...
        if (!IsRectEmpty (&bound)) {
            if (IntersectRect (&bound, &bound, &g_rcScr)) {
                ...
                if(IntersectRect (&bound, &bound, &rect)){
					...
					printf("%s %d %d %d %d %d\n",__FUNCTION__,__LINE__,bound.left,bound.right,bound.top,bound.bottom);
                    header->dirty = TRUE;
                }
                //header->dirty_rect = bound;
                //header->dirty = TRUE;
                ...
                MLSHADOW_REPLY_SURFACE_CREATE* reply)
{
    ...
    if (!GAL_ReallocFormat (surface, depth, real_vformat.Rmask, real_vformat.Gmask,
                        real_vformat.Bmask, real_vformat.Amask))
        return -1;
    ...
    if(video_mem_info.type == MLS_FB_TYPE_VIDEOMEM) {
        ...
        if (fb_size < (video_mem_info.video_mem_len - video_mem_info.video_mem_offset)) {
            ...
            header = shmat(reply->shm_id_header, 0, 0);
			//memset(header,0,sizeof(MLShadowFBHeader));
            surface->pixels  = _mmaped_mem + video_mem_info.video_mem_offset;
    ...
    if(video_mem_info.type == MLS_FB_TYPE_SYSTEMMEM || !videomem_enough) {
		printf(" %s %d MLS_FB_TYPE_SYSTEMMEM\n",__FUNCTION__,__LINE__);
...
#include <linux/vt.h>
						...
						vinfo.yoffset =(vinfo.yoffset == 0) ? 1920 : 0;
            ...
            list_for_each_safe(me, back, &mlshadow_list){
                ...
                //if(slave_node->enabled)
                {
                    ...
                    if (header->dirty == TRUE) {
						...
						printf("%s %d %d %d %d %d\n",__FUNCTION__,__LINE__,dirty_rect.left,dirty_rect.right,dirty_rect.top,dirty_rect.bottom);
                        lock_fbh(header);
                        SetRect (&header->dirty_rect, 0, 0, 0, 0);
                        header->dirty = FALSE;
                        unlock_fbh(header);
