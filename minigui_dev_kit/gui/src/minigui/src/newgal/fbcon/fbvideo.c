...
#define FBACCEL_SIGMASTAR
#ifdef FBACCEL_SIGMASTAR
...
//#ifdef __TARGET_STB810__
    ...
    //else
    //current->pixels = mapped_mem + mapped_offset+vinfo.yres*finfo.line_length;
...
{
    while(1) {
        __mg_os_time_delay(20000);
        vidmem_bucket *bucket = (vidmem_bucket *)this->screen->hwdata;

        if(bucket->dirty) {
            RECT dirtyRect;
            dirtyRect = bucket->dirty_rect;

            bucket->dirty = FALSE;
            fprintf(stderr, "%d %d %d %d \n", dirtyRect.left, dirtyRect.top, dirtyRect.bottom, dirtyRect.right);

            //trylock_scr_sem();
            lock_fbh(bucket->semid);

            swap_surface(this, GAL_VideoSurface, canvas, TRUE);
            //refresh_fb(this, &dirtyRect,TRUE);
            unlock_fbh(bucket->semid);
        }
    }
