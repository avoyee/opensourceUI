...
BOOL InitNativeInput (INPUT* input, const char* mdev, const char* mtype)
{
    ...
    if (strcasecmp (mtype, "touchscreen") == 0) {
        mousedev = &mousedev_TOUCHSCREEN;
        goto found;
    }
    else if(strcasecmp (mtype, "tslib") == 0) {
        mousedev = &mousedev_TSLIB;
        goto found;
    }
#endif
...
found:
    fprintf (stderr, "IAL Native Engine: mtype found %s\n",mtype);
    if (mousedev) {
