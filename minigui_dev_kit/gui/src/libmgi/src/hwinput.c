...
/*
 * $Id: hwinput.c 301 2010-03-23 06:19:26Z rlzheng $
...
static void sendchar (PHWINFO phwi, int n)
{
    if (phwi->fitchar[n][2]) {
        WPARAM ch;
        ch = ((Uint8)phwi->fitchar[n][0])
            | (((Uint8)phwi->fitchar[n][1])<<8)
            | (((Uint8)phwi->fitchar[n][2])<<16);
#if !defined(_LITE_VERSION) || defined(_STAND_ALONE) //pthread
        SendMessage (GetActiveWindow(), MSG_CHAR, ch, (LPARAM)FROM_HWINPUT);
#else
#if (MINIGUI_MAJOR_VERSION > 1)
        Send2ActiveWindow (mgTopmostLayer, MSG_CHAR, ch, (LPARAM)FROM_HWINPUT);
#else
        Send2ActiveClient (MSG_CHAR, ch, (LPARAM)FROM_HWINPUT);
#endif
#endif
    } else if (phwi->fitchar[n][1]) {
    ...
    memset (phwi->fitchar[0], 0, phwi->nword *4);
    memset (phwi->result, 0, phwi->nword*3); 
...
static int hw_init_recog_engine (PHWINFO phwi)
{
    ...
    if (phwi->engine->init)
        phwi->engine->init(phwi->width, phwi->height);
...
void recognize_draw (PHWINFO phwi)
{
    ...
    if ((rc = phwi->engine->recognize (phwi->m_data,  
        ...
        for (i = 0; i < phwi->nword; i++) {
            phwi->fitchar[i][0] = phwi->result[i*3];
            phwi->fitchar[i][1] = phwi->result[i*3 + 1];
            phwi->fitchar[i][2] = phwi->result[i*3 + 2];
            phwi->fitchar[i][3] = '\0';
        }
    }
}
...
static void* start_hw_ime (void* data)
{
    ...
    phwi->width = hw_ime_info->hw_info->w;
    phwi->height = hw_ime_info->hw_info->h;
