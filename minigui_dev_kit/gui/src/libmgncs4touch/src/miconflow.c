...
static void mIconFlow_drawItem (mIconFlow *self, HITEM hItem, HDC hdc, RECT *rcDraw)
{
    ...
    //pdc = PDC(self->bmpFrameDC)


    //hdc_bmp =GetSubDC(self->bmpFrameDC, 0, 0,W, H);
    //hdc_icon = CreateCompatibleDCEx (hdc, W, H);
    hdc_icon = CreateSubMemDC1(self->bmpFrameDC, 0, 0,W,H,TRUE);
    /*
    hdc_bmp = CreateMemDCEx(w, h, pdc->surface->format->BitsPerPixel,  pdc->surface->flags,
                            pdc->surface->format->Amask, pdc->surface->format->Gmask, pdc->surface->format->Bmask, pdc->surface->format->Amask,
                             pdc->surface->pixels, pdc->surface->pitch);
    */
/*
    if (self->iconUseZoom)
...
*/
    hdc_bmp =CreateMemDCFromBitmap(hdc,bmp);
...
static void mIconFlow_destroy (mIconFlow *self)
{
    ...
    if (self->bmpFrameDC != HDC_INVALID && self->bmpFrameDC != HDC_SCREEN)
        DeleteCompatibleDC (self->bmpFrameDC);
...
mIconFlow_construct (mIconFlow *self, DWORD add_data)
{
    ...
    self->bmpFrameDC = HDC_INVALID;
    self->drawBuffDC = HDC_INVALID;
    self->isVertical = FALSE;
    self->iconBorder = 0;
    self->span = DEF_ICON_WIDTH *2;
...
mIconFlow_onCreate (mIconFlow *self, LPARAM lParam)
{
    ...
    dc = GetClientDC (self->hwnd);
    self->drawBuffDC = CreateCompatibleDC (dc);
    self->bmpFrameDC = CreateCompatibleDC (dc);
    //self->iconFrameDC = CreateCompatibleDC (dc);
    ReleaseDC (dc);
