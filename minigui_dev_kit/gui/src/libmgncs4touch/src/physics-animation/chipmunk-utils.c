...
cp_baffle_board_t *create_baffle_board(cpSpace *space, int y_baseline, int y_height, int restLength, int x1, int x2, float stiffness, float damping) {
    ...
    board->shapes[0]->e = 0; board->shapes[0]->u = 0; board->shapes[0]->type = 1;
    ...
    /* Seg */
    board->bodies[1] = cpBodyNew(0, INFINITY);
    ...
    board->shapes[1]->e = 0; board->shapes[1]->u = 0; board->shapes[1]->type = 1;
    cpSpaceAddShape(space, board->shapes[1]);
...
cpShape *create_floor(cpSpace *space, int y_baseline, int x1, int x2) {
    ...
    floorBody = cpBodyNew(0, INFINITY);
...
cpShape *create_block(cpSpace *space, int x1, int y1, int x2, int y2, int m) {
    ...
    blockShape = cpPolyShapeNewRaw(blockBody, sizeof(verts)/sizeof(verts[0]), verts, radius);
