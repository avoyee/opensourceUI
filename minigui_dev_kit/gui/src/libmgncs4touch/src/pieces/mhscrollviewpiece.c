    ...
    switch(self->m_movingStatus) {
        ...
        case -1:
            if(v > 0 && p1 >= 0) {
                ...
                cpBodySetPosition(block->body, cpv(0, block->body->p.y));
        ...
        case 1:
            if(v < 0 && p2 <= L) {
                cpBodySleep(block->body);
                cpBodySetPosition(block->body, cpv(L - (p2 - p1), block->body->p.y));
                self->m_movingStatus = 0;
                needStop = 1;
            }
...
static cpSpace *s_setupSpace(mHScrollViewPiece *self, float v_x, float v_y)
{
    ...
    cpBodySetVelocity(shape->body, cpv(-v_x, 0));
    cpSpaceSetSleepTimeThreshold(space, 30000.0f);
...
static int s_onMouseMoveIn(mHotPiece *_self, int message, WPARAM wParam, LPARAM lParam, mObject *owner)
{
    if(((BOOL)wParam)) {
        ...
        s_onMouseRelease(_self, MSG_LBUTTONUP, wParam, lParam, owner);
    }
